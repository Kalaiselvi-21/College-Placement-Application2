...
const isStudentOrPR = (user) => {
  const normalized = user?.roleNormalized || String(user?.role || '').trim().toLowerCase().replace(/\s+/g, '_');
...
router.post('/consent', auth, upload.single('signature'), async (req, res) => {
  try {
    ...
    // Use NeonDB only
    logger.logAttempt('NEON', 'READ', 'User', `Fetching user for consent submission: ${req.user.id}`);
    ...
    logger.logSuccess('NEON', 'READ', 'User', 'User found for consent', req.user.id);
    ...
    try {
      logger.logAttempt('NEON', 'UPDATE', 'User', `Saving consent and OTP for user`);
      ...
      logger.logSuccess('NEON', 'UPDATE', 'User', `Consent saved in ${saveDuration}ms`, req.user.id);
...
router.post('/verify-otp', auth, async (req, res) => {
  try {
    ...
    // Use NeonDB only
    logger.logAttempt('NEON', 'READ', 'User', `Fetching user for OTP verification: ${req.user.id}`);
    const user = await neonService.findUserById(req.user.id);
    ...
    const duration = Date.now() - startTime;
    logger.logSuccess('NEON', 'READ', 'User', 'User found for OTP verification', req.user.id);
    logger.logPerformance('read', 'User', duration, 'NeonDB');

    console.log('User found in NeonDB');
    console.log('Verification Status:', JSON.stringify(user.verificationStatus, null, 2));
    ...
    logger.logAttempt('NEON', 'UPDATE', 'User', `Updating OTP verification status for user: ${user.email}`);
    const saveStartTime = Date.now();
    logger.logSuccess('NEON', 'UPDATE', 'User', `OTP verified and saved`, user.id);
    const saveDuration = Date.now() - saveStartTime;
    logger.logPerformance('UPDATE', 'User', saveDuration, 'NeonDB');
...
router.post('/resend-otp', auth, async (req, res) => {
  try {
    ...
    // Use NeonDB only
    logger.logAttempt('NEON', 'READ', 'User', `Fetching user for resend OTP: ${req.user.id}`);
    const user = await neonService.findUserById(req.user.id);
    ...
    logger.logSuccess('NEON', 'READ', 'User', 'User found for resend OTP', req.user.id);
    ...
    if (user.verificationStatus?.lastOtpSent) {
      const timeSinceLastOtp = Date.now() - new Date(user.verificationStatus.lastOtpSent).getTime();
    ...
    logger.logAttempt('NEON', 'UPDATE', 'User', `Updating OTP for resend: ${user.email}`);
